...
import {container, searchContainer, header, searchBoxContainer,text, button, image, searchbox, resultBlock, addButton, innerShadow, dropShadow, dropShadowInput, glassComponent} from '../style/style.js';
...
const showResult=()=>{
     return results.map((result,i)=>{
        ...
        <View key={i} style={styles.image}>
          ...
          {fontsLoaded&&(<Text style={{fontFamily:'Montserrat-Regular',color:'white',fontSize:18}}>{result.Title.length > 25 ? result.Title.substring(0, 15) + "..." : result.Title}</Text>)}
          ...
          <View style={[styles.addButton, styles.dropShadow]}>
          ...
          <View style={[styles.addButton, styles.dropShadow]}><Button onPress={()=>handleSubmit(result.imdbID)} title="+" color="grey"/></View>
          </ImageBackground>
         )}
        </View>
      )})}
...
////////////////////////////////////////// FOR RETURN //////////////////////////////////////////
  ...
  <LinearGradient colors={['#192b87', '#5dbdf5']} style={styles.container}>
    <SafeAreaView>
        ...
        <View style={styles.searchBoxContainer}>
          <View style={styles.searchbox}>{fontsLoaded && <TextInput style={{ fontFamily: 'Montserrat-Light' }} value={s} placeholder="search for a movie" onChangeText={(text) => setS(text)}/>}<View style={[styles.addButtonSearch, styles.dropShadow]}><Button onPress={()=>searchFilm(s)} title="+" color="grey"/></View></View>
        </View>
...
const styles = StyleSheet.create({
  ...
  addButton:addButton,
  addButtonSearch: {...addButton, marginRight:-17, marginTop:-20, zIndex:20000000000 },
